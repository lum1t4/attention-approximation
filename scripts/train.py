...
def training_step(state: TrainContext, step: int):
    ...
    print0(f"step {step+1:4d}/{state.config.max_steps} | train loss {cum_loss:.6f} | ce loss {cum_ce_loss:.6f} | kl loss {cum_kl_loss:.6f} | norm {norm:.4f} | lr {lr:.2e} | ({(dt)*1000:.2f} ms | {tokens_per_second:.0f} tok/s)")
    return state
